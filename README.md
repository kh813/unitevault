...
iCloudは、同じファイルをオフラインで別々に編集するなど、自動で統合できない変更を検出すると、`ノート名 (Macの競合コピー).md`や`ノート名 (1).md`のような別名の複製ファイルを作成することがあります。Aモードでは、タスクトレイ／メニューバーの **[ Check for Conflicts and Merge ]** から、このような複製ファイルを検出して元ファイルとマージできます（手動実行のみ、バックグラウンドでは自動的に行いません）。詳細は下記「iCloudの競合コピーが作られた場合」を参照してください。
...
**初回起動時のみ、免責事項ダイアログが表示されます。** UniteVaultが非公式・無保証のツールであること、Vaultファイルを自動的に移動・マージ・同期することへの同意を求める内容です。**[ I Agree ]** を押すまで先には進めません（**[ Quit ]** を押すとアプリが終了します）。一度同意すれば、その端末では二度と表示されません。

初回起動時は未設定のため、上記に同意すると**Settingsウィンドウが自動的に開きます**（メニューバー／タスクトレイアイコンをクリックし、「Settings」からいつでも再度開けます）。
...
- **Check for Conflicts and Merge**: iCloudの競合コピーを検出・マージ（Aモードかつ端末でVaultが設定済みの場合のみ選択可能、それ以外はグレーアウト。詳細は下記「iCloudの競合コピーが作られた場合」を参照）
- **Settings**: 設定の確認・変更
- **Check for Update**: 新しいバージョンがGitHub Releasesに公開されていないか確認し、あればダウンロード・自動適用・再起動まで行う（下記「アップデート方法」を参照。手動チェックに加え、週1回程度バックグラウンドでも自動的にチェックされます）
- **About UniteVault**: 現在インストール済みのバージョンを表示
- **Quit UniteVault**: 終了
...
メニューバー／タスクトレイの **[ Check for Update ]** から、新しいバージョンがGitHub Releasesに公開されていないか、いつでも手動で確認できます。加えて、UniteVaultは**週1回程度**、ユーザー操作なしでバックグラウンドからも自動的に同じチェックを行います（最終チェック日時はディスクに保存され、アプリを毎日終了・再起動するような使い方でも間隔がリセットされません）。

- 新しいバージョンがある場合、手動・自動どちらのチェックでも同じ確認ダイアログが表示され、同意した場合のみダウンロード・適用・再起動まで行われます（自動チェックがサイレントに適用することはありません）。
- 自動チェックで最新版が無い場合は何も表示されません（手動チェックの場合のみ「最新版です」と表示されます）。チェック自体が失敗した場合（ネットワーク不通等）も何も表示されず、次回のタイミングで再試行されます。
...
これに気づいたら、タスクトレイ（Windows）／メニューバー（Mac）の **[ Check for Conflicts and Merge ]** を選んでください。Aモードでかつ端末でVaultが設定済みの場合のみ選択できます（それ以外の場合はグレーアウトされます）。
...
[MIT License](LICENSE)です。コードのコピー・改変・再配布・フォークは自由ですが、本ソフトウェアは無保証で提供され、ファイル操作やデータの取り扱いに起因する不具合・損害について作者は一切の責任を負いません。同じ内容の免責事項が、**初回起動時に同意ゲート（「同意する」を押さないと利用を開始できないダイアログ）としても表示されます**（端末ごとに1回のみ）。
