...
- **最新版が無い場合**: 「You're running the latest version」を表示して終了する。
...
**自己更新の適用方式（OSごと）**：自プロセスは実行中の自分自身を安全に置き換えられないため、実際のファイル置換・再起動は**検知用に切り離したヘルパープロセス**（macOS: シェルスクリプト、Windows: PowerShellスクリプト）に委譲する。呼び出し元のGoプロセスはヘルパー起動後、直ちに終了する。
...
- **Windows**: 実行中の`.exe`を直接上書きできないため、新しい`.exe`を`UniteVault.exe.new`として同一フォルダにダウンロードし、`Start-Sleep`→`Rename-Item`（旧→`.old`、新→正式名）→`Start-Process`（再起動）を行うPowerShellスクリプトを一時ファイルとして書き出し、`-File`と名前付きパラメータ（`-ExePath`等）でパスを渡してデタッチ起動する（PowerShellコマンド文字列へのパス埋め込みは行わない）。
