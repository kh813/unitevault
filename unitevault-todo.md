...
- [x] cron / launchd / タスクスケジューラから呼び出せるよう、1回実行して終了するバッチ処理として設計する
...
- [x] サブコマンド構成の決定・実装（例：`unitevault init`, `unitevault run`, `unitevault status`, `unitevault promote`（プライマリ移行））
- [x] フラグ・環境変数によるVaultパス等の指定
- [x] Phase 8のエンジンをコマンドから呼び出す配線
- [x] `--help` 出力の整備
